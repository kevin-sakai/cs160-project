...
export default function Graphs() {
  ...
  function makeBar(labels, data, labelText, color) {
    return {
      labels,
      datasets: [
        {
  ...
  function normalizeToPercentages(data) {
    const total = data.reduce((sum, val) => sum + val, 0); //accumulate
    if (total === 0) return data;
    return data.map((val) => Math.round((val / total) * 100)); //percentage
  }
  ...
  return (
          ...
          <div className="chart-card">
            ...
            <Bar
              data={makeBar(
                ...
                'Usage Count',
                "rgb(251, 251, 251)" //hardcoded highlight
              )}
            />
          </div>
        )}
    ...
    <section className="twitch-sentiment-section">
      ...
      <div className="twitch-controls">
        ...
        >
          {sentimentLoading
            ? 'Connecting...' //sentloading true
            : twitchConnected
            ? 'Connected' //sentloading false, twitchconnected true
            : 'Connect to Twitch'}
        </button>
        {twitchConnected && (
        ...
        <div className="chart-blocks">
          <div className="chart-card">
            ...
            <div className="sentiment-stats">
              <p>
                <strong>Total Messages Analyzed:</strong>{' '}{sentimentData.stats.totalMessages}
              </p>
